...
| `question` | 執行中提問 | 框架核心功能，設為 `allow` 會繞過詢問保護機制 |

### 2.2 為什麼不能在插件內設定

這些權限**雖然可以設定於 `opencode.jsonc`**（供框架全域使用），**但不能在插件內設定給 Shadow Agent 使用**：

- 這些鍵值屬於 **OpenCode 框架的內部安全機制**
- 在插件內設定這些權限會導致設定衝突，使整個系統崩潰

### 2.3 為什麼這些鍵值會導致崩潰

這些鍵值不同於一般的 agent 工具權限：
...
4. **`question`** — 框架的核心詢問功能
   - 設為 `allow` 會繞過詢問保護機制，讓 agent 可以任意發問
   - 可能導致非預期的使用者介面彈出
