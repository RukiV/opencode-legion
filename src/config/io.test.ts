...
describe("isPluginRegisteredFromResult", () =>
{
	test("已註冊時返回 true", () =>
	{
		const result = hasPlugin(["@RukiV/opencode-legion"], [PLUGIN_NAME, LEGACY_PLUGIN_NAME] as const);
	...
	{
		const result = hasPlugin(["opencode-legion"], [PLUGIN_NAME, LEGACY_PLUGIN_NAME] as const);
		const isRegistered = isPluginRegisteredFromResult(result, PLUGIN_NAME);
...
describe("getLegacyPluginNamesFromResult", () =>
{
	test("有舊版插件時返回陣列", () =>
	{
		...
		expect(legacyNames).toEqual(["opencode-legion"]);
	});
	...
	test("同時有新舊插件時只返回舊版", () =>
	{
		const pluginList = ["opencode-legion", "@RukiV/opencode-legion"];
	...
	test("多個舊版插件時返回所有匹配", () =>
	{
		const pluginList = ["opencode-legion", "opencode-legion@1.0.0"];
		const result = hasPlugin(pluginList, [PLUGIN_NAME, LEGACY_PLUGIN_NAME] as const);
		const legacyNames = getLegacyPluginNamesFromResult(result);

		/** 返回所有只匹配舊版名稱的插件 / Returns all plugins only matching legacy name */
		expect(legacyNames).toEqual(["opencode-legion", "opencode-legion@1.0.0"]);
	});
...
describe("isPluginNameMatch", () =>
{
	test("精確匹配時返回 true", () =>
	{
		expect(isPluginNameMatch("opencode-legion", "opencode-legion")).toBe(true);
	});

	test("不匹配時返回 false", () =>
	{
		expect(isPluginNameMatch("other-plugin", "opencode-legion")).toBe(false);
	});

	test("scoped package 精確匹配", () =>
	{
		expect(isPluginNameMatch("@username/opencode-legion", "@username/opencode-legion")).toBe(true);
	});
	...
	test("scoped package 不匹配 unscoped", () =>
	{
		 ...
		 *
		 * "@username/opencode-legion" !== "opencode-legion"
		 */
		expect(isPluginNameMatch("@username/opencode-legion", "opencode-legion")).toBe(false);
		expect(isPluginNameMatch("opencode-legion", "@username/opencode-legion")).toBe(false);
	});
});
...
describe("hasPlugin", () =>
{
	...
	test("單一插件名稱匹配", () =>
	{
		const pluginList = ["@RukiV/opencode-legion"];
		const result = hasPlugin(pluginList, [PLUGIN_NAME]);
		/** 返回匹配的插件陣列 / Returns array of matched plugins */
		expect(result[PLUGIN_NAME]).toEqual(["@RukiV/opencode-legion"]);
	});

	test("多個插件中只有一個匹配", () =>
	{
		const pluginList = ["other-plugin", "@RukiV/opencode-legion", "another-plugin"];
		const result = hasPlugin(pluginList, [PLUGIN_NAME]);
	...
	test("舊版插件名稱匹配", () =>
	{
		const pluginList = ["opencode-legion"];
		const result = hasPlugin(pluginList, [LEGACY_PLUGIN_NAME]);
		expect(result[LEGACY_PLUGIN_NAME]).toEqual(["opencode-legion"]);
	});
	...
	test("陣列形式的插件名稱查詢", () =>
	{
		const pluginList = ["plugin-a", "plugin-b", "@RukiV/opencode-legion"];
...
describe("checkPluginRegistration 邏輯測試", () =>
{
	...
	test("舊版插件檢測邏輯 - 只有舊版名稱", () =>
	{
		...
		expect(legacyPluginNames).toEqual(["opencode-legion"]);
	});
	...
	test("舊版插件檢測邏輯 - 有新舊插件同時存在", () =>
	{
		...
		/** "opencode-legion" 保留因為它只匹配舊版名稱 / "opencode-legion" is kept because it only matches legacy name */
		expect(legacyPluginNames).toEqual(["opencode-legion"]);
	});
