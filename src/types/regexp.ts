 ...
 */
export const HIGH_LOAD_PATTERN: RegExp = new zhRegExpWithPluginEnabled('under\\s+high\\s+load|retry.+(?:after|wait)|please.+wait|高負載|後重試|再試行|高負荷|Provider\\s*(?:returned\\s*)?error|rate.*limit(?:ed|s)?|(?:Cannot|Unable).*connect', 'i');
